...
public class MagmaReflectionHandler extends ClassLoader {
    ...
    public static String handlePackageGetName(String name) {
        if (name.startsWith(PREFIX)) {
            return PREFIX + "server." + Magma.getBukkitVersion();
        } else {
            return name;
        }
    }
    ...
    public static Object[] handleMethodInvoke(Method method, Object src, Object[] param) throws Throwable {
        Object[] ret = MagmaRedirectAdapter.runHandle(remapper, method, src, param);
        if (ret != null) {
            return ret;
        } else {
    ...
    public static Object redirectMethodInvoke(Method method, Object src, Object[] param) throws Throwable {
        Object ret = MagmaRedirectAdapter.runRedirect(remapper, method, src, param);
        if (ret != remapper) {
            return ret;
        } else {
    ...
    public static byte[] transformOrAdd(ClassLoader loader, byte[] bytes) {
        ...
        if (rcl != null) {
            ...
            MagmaRedirectAdapter.scanMethod(bytes);
            return bytes;
        }
    }
}
