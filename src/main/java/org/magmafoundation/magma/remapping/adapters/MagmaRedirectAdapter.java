 ...
 * @classFrom <a href="https://github.com/IzzelAliz/Arclight/blob/FeudalKings/arclight-common/src/main/java/io/izzel/arclight/common/mod/util/remapper/ArclightRedirectAdapter.java">Click here to get to github</a>
 *            <p>
...
public class MagmaRedirectAdapter implements PluginTransformer {
    ...
    private static void addRule(boolean modifyArgs, Class<?> owner, String name, String handlerName, Class<?>... args) {
        ...
        Method handler = methodOf(MagmaReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        while (handler == null) {
            handlerArgs[0] = handlerArgs[0].getSuperclass();
            handler = methodOf(MagmaReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        }
        ...
        if (modifyArgs) {
            Method modifyHandler = methodOf(MagmaReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            if (modifyHandler == null) {
                handlerArgs[0] = original.getReturnType();
                modifyHandler = methodOf(MagmaReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            }
            if (modifyHandler == null) {
