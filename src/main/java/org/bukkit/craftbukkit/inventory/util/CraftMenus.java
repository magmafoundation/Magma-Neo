...
public final class CraftMenus {
    ...
    public static <V extends InventoryView> MenuTypeData<V> getMenuTypeData(CraftMenuType<?> menuType) {
        ...
        if (menuType == MenuType.GENERIC_3X3) {
            return asType(new MenuTypeData<>(InventoryView.class, CraftMenuBuilder.tileEntity(DispenserBlockEntity::new, Blocks.DISPENSER)));
        }
        if (menuType == MenuType.CRAFTER_3X3) {
            return asType(new MenuTypeData<>(CrafterView.class, CraftMenuBuilder.tileEntity(CrafterBlockEntity::new, Blocks.CRAFTER)));
        }
        if (menuType == MenuType.ANVIL) {
            return asType(new MenuTypeData<>(AnvilView.class, CraftMenuBuilder.worldAccess(AnvilMenu::new)));
        }
        if (menuType == MenuType.BEACON) {
            return asType(new MenuTypeData<>(BeaconView.class, CraftMenuBuilder.tileEntity(BeaconBlockEntity::new, Blocks.BEACON)));
        }
        if (menuType == MenuType.BLAST_FURNACE) {
            return asType(new MenuTypeData<>(FurnaceView.class, CraftMenuBuilder.tileEntity(BlastFurnaceBlockEntity::new, Blocks.BLAST_FURNACE)));
        }
        if (menuType == MenuType.BREWING_STAND) {
            return asType(new MenuTypeData<>(BrewingStandView.class, CraftMenuBuilder.tileEntity(BrewingStandBlockEntity::new, Blocks.BREWING_STAND)));
        }
        if (menuType == MenuType.CRAFTING) {
            return asType(new MenuTypeData<>(InventoryView.class, CraftMenuBuilder.worldAccess(CraftingMenu::new)));
        }
        if (menuType == MenuType.ENCHANTMENT) {
            return asType(new MenuTypeData<>(EnchantmentView.class, (player, type) -> {
                return new SimpleMenuProvider((syncId, inventory, human) -> {
                    return CraftMenuBuilder.worldAccess(EnchantmentMenu::new).build(player, type);
                }, Component.empty()).createMenu(player.nextContainerCounterInt(), player.getInventory(), player);
            }));
        }
        if (menuType == MenuType.FURNACE) {
            return asType(new MenuTypeData<>(FurnaceView.class, CraftMenuBuilder.tileEntity(FurnaceBlockEntity::new, Blocks.FURNACE)));
        }
        if (menuType == MenuType.GRINDSTONE) {
            return asType(new MenuTypeData<>(InventoryView.class, CraftMenuBuilder.worldAccess(GrindstoneMenu::new)));
        }
        ...
        if (menuType == MenuType.HOPPER) {
            return asType(new MenuTypeData<>(InventoryView.class, CraftMenuBuilder.tileEntity(HopperBlockEntity::new, Blocks.HOPPER)));
        }
        ...
        if (menuType == MenuType.LECTERN) {
            return asType(new MenuTypeData<>(LecternView.class, CraftMenuBuilder.tileEntity(LecternBlockEntity::new, Blocks.LECTERN)));
        }
        ...
        if (menuType == MenuType.SMITHING) {
            return asType(new MenuTypeData<>(InventoryView.class, CraftMenuBuilder.worldAccess(SmithingMenu::new)));
        }
        if (menuType == MenuType.SMOKER) {
            return asType(new MenuTypeData<>(FurnaceView.class, CraftMenuBuilder.tileEntity(SmokerBlockEntity::new, Blocks.SMOKER)));
        }
        if (menuType == MenuType.CARTOGRAPHY_TABLE) {
            return asType(new MenuTypeData<>(InventoryView.class, CraftMenuBuilder.worldAccess(CartographyTableMenu::new)));
        }
        if (menuType == MenuType.STONECUTTER) {
            return asType(new MenuTypeData<>(StonecutterView.class, CraftMenuBuilder.worldAccess(StonecutterMenu::new)));
        }
